...
export function Hero({ isAuthenticated }: HeroProps) {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-background text-foreground">
      ...
      <div className="relative mx-auto grid min-h-[100svh] max-w-6xl items-start gap-8 px-6 pt-8 pb-16 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:gap-12 md:pt-16 md:pb-32 lg:gap-20">
        <div className="flex flex-col items-start text-left">
          ...
          <h1 className="mt-6 text-balance text-4xl font-black leading-[1.05] sm:text-6xl lg:text-7xl">
            Tu yo digital, conectado a todas tus apps.
          </h1>
          <p className="mt-4 max-w-xl text-lg text-muted-foreground sm:mt-6 sm:text-2xl">
          ...
          <div className="mt-6 flex w-full flex-col gap-3 sm:mt-8 sm:w-auto sm:flex-row sm:flex-wrap">
            <Button
              asChild
              size="lg"
              className="h-12 w-full px-6 text-base sm:h-14 sm:w-auto sm:px-8 sm:text-lg bg-accent text-accent-foreground hover:bg-accent/90"
            >
            ...
            <Button
              ...
              size="lg"
              variant="ghost"
              className="h-12 w-full px-4 text-base sm:h-14 sm:w-auto sm:px-6 sm:text-lg text-foreground hover:!bg-foreground/10 hover:!text-foreground"
            >
              <Link href={secondary.href}>{secondary.label}</Link>
            </Button>
          </div>
          <p className="mt-4 text-sm text-muted-foreground/80 sm:mt-6 sm:text-lg">
            Tus datos quedan bajo tu control en todo momento.
          </p>
        </div>

        <div className="mt-2 flex justify-center md:mt-0 md:self-center">
